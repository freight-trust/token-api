...
import { ClientStakingGetstakingreward } from "./client/block/Getblockreward";
...
export class Client {
  ...
  proxy(action: string) {
    ...
    const actions: { [key: string]: any } = {
      ...
      eth_getStorageAt(address: string, position: string, tag: string) {
        const client = new ClientProxyEthGetStorageAt(
          new Address(address),
          position,
          tag
        );
      ...
      eth_getTransactionByBlockNumberAndIndex(index: string, tag: string) {
        const client = new ClientProxyEthGetTransactionByBlockNumberAndIndex(
          index,
          tag
        );
        const json = client.toJson();
      ...
      eth_getTransactionCount(address: string, tag: string) {
        const client = new ClientProxyEthGetTransactionCount(
          new Address(address),
          tag
        );
        const json = client.toJson();
      ...
      eth_getUncleByBlockNumberAndIndex(index: string, tag: string) {
        const client = new ClientProxyEthGetUncleByBlockNumberAndIndex(
          index,
          tag
        );
        const json = client.toJson();
  ...
  stats(action: string): any {
    ...
    const actions: { [key: string]: any } = {
      ...
      chainsize(
        ...
        const client = new ClientStatsChainsize(
          oStartdate,
          oEnddate,
          oClienttype,
          oSyncmode
  ...
  block(action: string): any {
    ...
    const actions: { [key: string]: any } = {
      getblockreward(blockno: string) {
        ...
        const client = new ClientStakingGetstakingreward(blocknoNumber);
        const json = client.toJson();
        json.apiKey = apiKey.toString();
        return performRequest(
          network,
          ClientStakingGetstakingreward.module,
          ClientStakingGetstakingreward.action,
          json
        ).then((response) => response.json());
      },
    };
  ...
  account(action: string): any {
    const validatingAction =
      action === "txlistinternaltxhash" ? "txlistinternal" : action;
    const apiKey = this.apiKey;
    const network = this.network;
    ...
    const actions: { [key: string]: any } = {
      ...
      balancemulti(address: string[], tag: string): Promise<any> {
        const oAddress = address.map(
          (addresString) => new Address(addresString)
        );
      ...
      getstakingblocks(address: string, type: string): Promise<any> {
        const oAddress = new Address(address);
      ...
      tokentx(
        ...
        const client = new ClientAccountTokentx(
          oAddress,
          startblock,
          endblock,
          oSort
      ...
      txlist(
        ...
        const paging =
          !!page && !!offset
            ? new Paging(new PositiveNumber(page), new PositiveNumber(offset))
            : undefined;
        const client = new ClientAccountTxlist(
          oAddress,
          startblock,
          endblock,
          oSort,
      ...
      txlistinternal(
        ...
        const oAddress = new Address(address);
        const oSort = !!sort ? new Sort(sort) : undefined;
        const client = new ClientAccountTxlistinternal(
          oAddress,
          startblock,
          endblock,
          oSort
      ...
      txlistinternaltxhash(
        ...
        const oSort = !!sort ? new Sort(sort) : undefined;
        const client = new ClientAccountTxlistinternalTxhash(
          txhash,
          startblock,
          endblock,
          oSort
